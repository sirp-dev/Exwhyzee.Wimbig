...
{
    public class NewSideBarModel : PageModel
    {
        private readonly IBarnerAppService _barnerAppService;
        private readonly IHostingEnvironment _hostingEnv;


        public NewSideBarModel(IBarnerAppService barnerAppService, IHostingEnvironment env)
        {
            _barnerAppService = barnerAppService;
            _hostingEnv = env;
        }
        
        public SideBarnerFile sideBarFIle { get; set; }
        [BindProperty]
                    ...
                    if (HttpContext.Request.Form.Files != null && HttpContext.Request.Form.Files.Count > 0)
                    {
                        ...
                        foreach (var file in files)
                        {
                            if (file.Length > 0)
                            {
                                ...
                                img.TargetLocation = LocationTarget;
                                var saveImageToDb = await _barnerAppService.InsertSideBarner(img);
