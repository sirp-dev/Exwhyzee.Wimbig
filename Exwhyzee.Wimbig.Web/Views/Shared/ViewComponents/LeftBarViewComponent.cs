...
namespace Exwhyzee.Wimbig.Web.Views.Shared.ViewComponents
{
    public class LeftBarViewComponent : ViewComponent
    {
        ...
        public LeftBarViewComponent(IBarnerAppService barnerAppService, IHostingEnvironment env)
        {
        ...
        private async Task<List<SideBarnerFile>> GetBarner()
        {
            ...
            return ticket.Source.Where(x => x.TargetLocation == "Side Bar Big").Skip(2).Take(2).ToList();
        }
        //
        private async Task<List<SideBarnerFile>> GetSmallBarner()
        {
            var ticket = await _barnerAppService.GetBarnerFileSideBarner();
            return ticket.Source.Where(x => x.TargetLocation == "Side Bar Small").Skip(1).Take(1).ToList();
        }
    }
}
